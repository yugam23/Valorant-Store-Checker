...
import { getWeaponSkinsByUuids, getContentTierByUuid } from "@/lib/valorant-api";
import { InventoryData, OwnedSkin } from "@/types/inventory";
...
export async function getOwnedSkins(tokens: StoreTokens): Promise<InventoryData> {
  ...
  log.debug(`Hydrating ${skinUuids.length} skins from Valorant-API`);
  const skinsMap = await getWeaponSkinsByUuids(skinUuids);

  // Build OwnedSkin objects
